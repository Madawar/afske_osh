<div>
    ...
    <div class="my-2 flex sm:flex-row flex-col justify-center bg-gray-100 p-2 shadow-sm">
        ...
        <a class="ml-2 inline-block px-6 py-2 text-xs font-medium leading-6 text-center text-white uppercase transition bg-blue-700 rounded shadow ripple hover:shadow-lg hover:bg-blue-800 focus:outline-none" href="{{route('checklist.create')}}"> New Checklist </a>
    ...
    <table class="table-auto w-full" wire:loading.class="cursor-wait">
        <thead>
            <tr class="bg-gray-400">
                ...
                <th wire:click.prevent="sortBy('date')"
                    class="pr-5 pl-5  border-r border-t border-gray-300 cursor-pointer">Checklist</th>
        ...
        <tbody>
            @foreach ($checklists as $checklist)
                <td>{{ $loop->index + 1 }}</td>
                <td class="p-3 border border-r border-gray-50">{{ $checklist->name }}</td>
                <td class="p-3 border border-r border-gray-50">{{ $checklist->created_at }}</td>
                <td>
                    <a href="{{route('checklist.edit', $checklist->id)}}">Edit</a>
                </td>
